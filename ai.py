...
def get_roughness(signal):
    # 신호가 변하는 지점(0->1 또는 1->0)의 횟수를 카운트
...
def main():
    ...
    st.markdown("수학적 수식과 가중치 연산 과정을 실시간으로 추적합니다.")
    st.divider()
    ...
    with col1:
        ...
        calc_idx = st.slider("분석 인덱스 선택", 1, len(raw)-2, 5)
